...
## Summary
在 HarmonyOS (HMOS) 平台上为 Tauri WebView 实现 `createPdf()` 方法，使用 `WebviewController.createPdf()` API 将当前加载的网页转换为 PDF 文件。支持用户自定义 PDF 配置参数，未提供时使用默认 A4 配置。
...
│  Rust 调用方                                                      │
│                                                                  │
│  webview.create_pdf(path, config, callback)  →  Result<()>       │
│    path: &str                                                    │
...
│    │  将 PdfConfig 序列化为 HashMap<String, Either<f64,bool>>    │
...
webview.create_pdf("/data/storage/landscape.pdf", Some(PdfConfig {
    width: Some(11.69),      // 横向 A4 宽度
    height: Some(8.27),      // 横向 A4 高度
    margin_top: Some(0.5),   // 0.5 英寸上边距
    margin_bottom: Some(0.5),
    margin_left: Some(0.3),
    margin_right: Some(0.3),
    should_print_background: Some(true),
}), move |success| {
    if success {
        println!("横向 PDF 生成成功");
    } else {
